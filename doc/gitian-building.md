...
*Setup instructions for a Gitian build of Predix Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Predix
...
- [Building Predix Core](#building-Predix-core)
...
Clone the git repositories for Predix and Gitian.
...
```bash
sudo cp /home/debian/Predix/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the Predix repository under 'Fetch and create inputs' to install sources which require
...
To build Predix Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Predix repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Predix/.git/
...
For example:
```bash
URL=https://github.com/laanwj/Predix.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit Predix=${COMMIT} --url Predix=${URL} ../Predix/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit Predix=${COMMIT} --url Predix=${URL} ../Predix/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit Predix=${COMMIT} --url Predix=${URL} ../Predix/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-Predix:
./bin/gbuild --commit Predix=${COMMIT},cpp-eth-Predix=${ETHCOMMIT} --url Predix=${URL},cpp-eth-Predix=${ETHURL} ../Predix/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the Predix git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../Predix/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/Predix
SIGPATH=/some/root/path/Predix-detached-sigs

./bin/gbuild --url Predix=${BTCPATH},signature=${SIGPATH} ../Predix/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Predix-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Predix-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/Predix-osx-build.assert
```
...
[Predixproject/gitian.sigs](https://github.com/Predixproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@Predix.org) and he will commit them.
