...
void OpenDebugLog()
{
    ...
    assert(fileout == NULL);
    assert(fileoutVM == NULL); // Predix
    assert(vMsgsBeforeOpenLog);
    boost::filesystem::path pathDebug = GetDataDir() / "debug.log";
    boost::filesystem::path pathDebugVM = GetDataDir() / "vm.log"; // Predix
    fileout = fopen(pathDebug.string().c_str(), "a");
    fileoutVM = fopen(pathDebugVM.string().c_str(), "a"); // Predix
    if (fileout) {
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Predix";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Predix";
#else
    // Unix
    return pathRet / ".Predix";
#endif
#endif
}
