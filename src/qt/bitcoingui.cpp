    ...
    platformStyle(_platformStyle)
{
    ...
    if(fCheckForUpdates && qtumVersionChecker->newVersionAvailable())
    {
        ...
        QString message(tr("New version of PDX wallet is available on the PDX source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a PDX address"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your PDX addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified PDX addresses"));
    ...
    openAction->setStatusTip(tr("Open a PDX: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible PDX command-line options").arg(tr(PACKAGE_NAME)));
...
{
    QString strTitle = tr("PDX"); // default title
